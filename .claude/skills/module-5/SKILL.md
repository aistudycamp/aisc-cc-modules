...
> "Welcome to Module 5! You've been learning how to shape Claude's behavior, give it skills, and extend it with plugins. Now we're going to do something really powerful: connect Claude to the outside world."
...
2. **The CLI** (Command Line Interface). The tool ships a terminal command — e.g. `npx instant-cli push`. Used both by humans who script and automate AND by coding agents like Claude Code itself — Claude can run any CLI command on your behalf, no MCP required.
...
> "You did it. You have a real, working MCP connection! Claude is wired into a tool you actually use. That's the moment MCP stops being a concept and starts being a superpower. Plus, you've got a wishlist of 4 more connections waiting for you whenever you're ready."
